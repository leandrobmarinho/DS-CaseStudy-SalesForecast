...
def filter_data(dflong: pd.DataFrame,
    ...
    if last_n is not None:
        logger.info(f"""Drop test data older than {last_n:,d} days old""")
...
def create_m5_fit_data(sales: pd.DataFrame,
    ...
    - sales data
    - calendar (date features thru test & forecast horizon
    ...
    # Here no filter_data is applied; the test data is processed in full
    # long = filter_data(long, last_n=last_n)
    last_date_test = long['date'].max()
    logger.debug(f"""Finished creating M5 fit data thru {last_date_test} in {time.time() - _start_time:,.1f} seconds""")
    return long
...
def create_future_features(
    ...
    """
    val_start = last_date_test
    val_end = last_date_test + pd.Timedelta(days=h)

    last_wmyrwk = cal[cal["date"] == last_date_test]["wm_yr_wk"].iloc[0]
...
if __name__ == "__main__":
    ...
    # LAST_N_DAYS is not defined here; the test data is processed in full.
    # LAST_N_DAYS = 400
